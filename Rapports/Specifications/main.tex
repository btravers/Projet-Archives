...
L'authentification comprend les fonctionnalités qui permettront aux utilisateurs de s'identifier afin d'accéder à leurs données propres ainsi qu'aux fonctionnalités auxquelles ils auront un accès d'utilisation. Par exemple, un utilisateur non identifié ne pourra pas annoter un document, tandis qu'un utilisateur identifié aurra l'autorisation d'annoter un document tel un registre matricule militaire ou une table des registres.
...
\subsubsection{Fonction SG01 - Gestion des utilisateurs}

Les utilisateurs sont divisés en trois groupes : les utilisateurs simples, les modérateurs et les administrateurs. Les utilisateurs simples sont les utilisateurs authentifiés de l'application et ont accès à tous les outils d'annotation. Les modérateurs sont des utilisateurs simples qui ont la possibilité de supprimer certains utilisateurs ou de désactiver la visibilité de leurs annotations. Enfin les administrateurs sont des modérateurs capables de changer le status des utilisateurs. Les administrateurs et les modérateurs n'étant pas obligatoirement des informaticiens, il est donc necéssaire de concevoir une interface pour la gestion des utilisateurs.
...
\begin{table}[H]
  \centering
   \small
	\begin{tabular}{|c|p{12cm}|}
   		\hline
   			\rowcolor{lightgray}\multicolumn{2}{|c|}{\textbf{Fonction SG01 - Gestion des utilisateurs}} \\
   		\hline
   			\multicolumn{2}{|l|}{Condition de déclenchement} \\
   		\hline
   			-1 & L'utilisateur a le status d'administrateur ou de modérateur.\\
        	-2 & L'utilisateur souhaite accéder à la page de gestion d'utilisateur.\\
   		\hline
   			\multicolumn{2}{|l|}{Déclenchement de l'opération} \\
   		\hline
   			-1 & L'utilisateur administrateur ou modérateur demande à accéder à la page de gestion des utilisateurs.\\
   		\hline
   			\multicolumn{2}{|l|}{Entrants de l'opération} \\
   		\hline
   			-1 & Aucun\\ 
   		\hline
   			\multicolumn{2}{|l|}{Traitement de l'opération} \\
  		\hline
   			-1 & Chargement des utilisateurs présents dans la base de données.\\
   		\hline
   			\multicolumn{2}{|l|}{Sortants de l'opération} \\
   		\hline
   		...
   		\hline
	\end{tabular}
  \caption{Fonction SG01 - Gestion des utilisateurs}
  \normalsize
  \label{tab: gestion_des_utilisateurs}
\end{table}

    
\subsubsection{Fonction SG02 - Supprimer un ou des utilisateurs}

Les modérateurs peuvent supprimer un utilisateur de la base. Ceci se fait au travers de la page de gestion des utilisateurs. Le modérateur sélectionne les utilisateurs qu'il souhaite supprimer puis il les supprime.
   ...
   \small
	\begin{tabular}{|c|p{12cm}|}
   		...
   		\hline
   			\multicolumn{2}{|l|}{Déclenchement de l'opération} \\
   		\hline
   			-1 & L'utilisateur modérateur sélectionne un ou des utilisateurs de la liste et les supprime.\\
   		\hline
   			\multicolumn{2}{|l|}{Entrants de l'opération} \\
   		\hline
  		...
  		\hline
   		...
   		\hline
	\end{tabular}
  \caption{Fonction SG02 - Supprimer un ou des utilisateurs}
  \normalsize
